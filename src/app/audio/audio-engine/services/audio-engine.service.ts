...
export interface PlayableClip {
  clipId: string; // Required for effect node caching
  buffer: AudioBuffer;
  ...
  effects?: ClipEffect[]; // Effect chain for this clip
}
  ...
  stop(): void {
    for (const n of this.scheduledNodes) {
      ...
      try {
        ...
        /* Gain node already disconnected */
      }
      try {
        n.pan.disconnect();
      } catch {
        /* Pan node already disconnected */
      }
    }
    this.scheduledNodes = [];

    // Dispose all effect chains
    this.effectsService.disposeAllChains();

    this.playing = false;
  }

  async renderToMp3(options: {
    clips: Iterable<PlayableClip>;
    duration: number;
    sampleRate?: number;
    bitRate?: number;
  }): Promise<Blob> {
    ...
    for (const c of processedClips) {
      ...
      gain.connect(pan).connect(off.destination);
